...
<div class="<?php echo $card_classes ?>"> <?php
    ...
    <div class="ekit-review-card--thumbnail <?php echo($thumbnail_badge ? 'ekit-review-card--thumbnail-badge' : '') ?>">
        <div>
            ...
            if($thumbnail_badge) { ?>
                <div class="badge">
                    <img src="<?php echo $handler_url . 'assets/svg/yelp-logo-circle.svg' ?>">
                </div> <?php
            } ?>
        </div>
    </div>
    ...
    <h5 class="ekit-review-card--name">
		<?php echo empty($item->user->name) ? esc_html__('Anonymous', 'elementsKit-lite') : $item->user->name; ?>
    </h5>
        ...
        <div class="ekit-review-card--posted-on">
            <?php 
                ...
                if ( $is_new || $migrated ) :
                    ...
                    <i class="<?php echo $ekit_yelp_review_posted_on_icon; ?>" aria-hidden="true"></i>
                <?php endif;
            ?>
            <div>
                <p class="small muted">
                    <?php echo esc_html__('Posted on', 'elementsKit-lite')?>
                </p>
                <h5 class='text-bold'><?php echo esc_html__('Yelp', 'elementsKit-lite')?></h5>
            </div>
        </div> <?php
