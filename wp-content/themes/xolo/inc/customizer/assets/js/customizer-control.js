...
( function( $ ) {
	...
	XOLOCustomizer = {
		...
		_initToggles: function()
		{
			...
			XOLOControlTrigger.triggerHook( 'xolo-toggle-control', api );

			// Loop through each setting.
			$.each( XOLOCustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'xolo-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'xolo-hide' );
									}
								}
							});
						});
