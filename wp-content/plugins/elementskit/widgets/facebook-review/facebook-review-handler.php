...
class ElementsKit_Widget_Facebook_Review_Handler extends \ElementsKit_Lite\Core\Handler_Widget {
	...
	static function get_title() {
		return esc_html__('Facebook review', 'elementsKit-lite');
	}
	...
	public static function get_fb_page_overall_rating($page_id, $pg_acc_token) {
		...
		try {
			...
			if(is_wp_error($request)) {
				...
				return array(
					'success' => false,
					'msg'     => __('API call failed to retrieve the facebook page info.', 'elementsKit-lite') . ' - ' . $request->get_error_message(),
				);
			}
			...
			if(empty($datum->name)) {
				...
				return array(
					'success' => false,
					'msg'     => __('API call failed to retrieve the facebook page info.', 'elementsKit-lite') . ' - ' . $ex->getMessage(),
				);
			}
