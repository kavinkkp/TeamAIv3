...
if(!class_exists('ElementsKit')){
	final class ElementsKit{
		...
		public function missing_elementskit() {
			...
			\Wpmet\Service\Notice::instance('elementskit', 'missing-elementskit-version')
			->add_class('notice-error')
			->add_title(sprintf( esc_html__( 'ElementsKit Pro requires ElementsKit Lite, which is currently NOT RUNNING. ', 'elementskit' )))
			->add_button([
				'url' => $btn['url'],
				'text' => $btn['text'],
