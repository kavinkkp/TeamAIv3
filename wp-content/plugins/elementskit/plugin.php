...
class Plugin{
    ...
    public function __construct() {
        ...
        if($license->status() != 'valid' && apply_filters('elementskit/license/hide_banner', false) != true){
            \Wpmet\Service\Notice::instance('elementskit', 'pro-not-active')
            ->add_class('notice-error')
            ->dismissible('global', (60*60*24*30))
            ->add_message(esc_html__('Please activate ElementsKit to get automatic updates, premium support and unlimited access to the layout library of ElementsKit.', 'elementskit'))
            ->add_button([
