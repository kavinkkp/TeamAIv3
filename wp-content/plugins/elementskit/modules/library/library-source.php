...
class Library_Source extends Source_Base {
	...
	public function get_title() {
		return __( 'Elementskit Library', 'elementskit' );
	}
	...
	public static function get_license(){
		if(!class_exists('\ElementsKit\Libs\Framework\Classes\Utils')){
			return [];
		}
		...
		return [
			...
			'oppai' => 1,
			'package_type' => \ElementsKit::PACKAGE_TYPE
		];
	}
	...
	public static function request_template_data( $template_id ) {
		...
		$request_url = \ElementsKit::api_url() . 'elementor-layouts/' . '?' . http_build_query($query);
	...
	public function get_data( array $args, $context = 'display' ) {
		...
		if ( empty( $data ) || empty( $data['content'] ) ) {
			throw new \Exception( __( 'Template does not have any content', 'elementskit' ) );
		}
